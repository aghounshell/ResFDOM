...
chem_inflow <- chem_inputs %>% 
  filter(Site==50 & depth == c(1.6,6.2)) 
...
ggsave("./Fig_Output/Fig4_v2_surfInflow.jpg",width=10,height=12,units="in",dpi=320)
...
# Plot CTD data and inflow temp to see where inflows go
ggplot()+
  geom_line(ctd_50_depths,mapping=aes(x=time,y=Temp_C,color=as.factor(depth)))+
  geom_line(inflow_daily,mapping=aes(x=DateTime,y=WVWA_Temp_C),color="black",size=1)+
  geom_line(inflow_daily,mapping=aes(x=DateTime,y=VT_Temp_C),color="black",size=1)+
...
ggsave("./Fig_Output/Fig6_v3_surfInflow.jpg",width=10,height=5,units="in",dpi=320)
...
ggsave("./Fig_Output/dmtodt_inflow_v2_surfInflow.jpg",width=10,height=8,units="in",dpi=320)
