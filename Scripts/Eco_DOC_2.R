...
# Following Hounshell et al. 2020
bathy <- read.csv("./Data/FCR_Bathy_Acoustic.csv")

### Interpolate data to bathy depths ----
chem_50_long <- chem_50 %>% 
  select(DateTime,Depth_m,DOC_mgL) %>% 
  pivot_wider(names_from = DateTime, values_from = DOC_mgL, values_fil = NA, values_fn = mean, names_prefix = "Date_") %>% 
  arrange(Depth_m)

bathy_depth <- bathy %>% 
  select(Depth_m)

chem_50_long <- left_join(bathy_depth,chem_50_long,by="Depth_m")
...
for(i in 2:length(doc_box_data$DateTime)){
  ...
  if(doc_box_data$epi_bottomg_depth_m[i] == doc_box_data$epi_bottomg_depth_m[i-1]){
    ...
    x = which(doc_box_data$DateTime[i]==doc_entr$DateTime & doc_box_data$epi_bottomg_depth_m[i]==doc_entr$depth)
    y = which(doc_box_data$DateTime[i]==doc_entr$DateTime & doc_box_data$hypo_top_depth_m[i]==doc_entr$depth)
    doc_box_data$DOC_entr_g[i] = sum(doc_entr$DOC_g[x:y])
  }
}
